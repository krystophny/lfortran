...
bool
startswith(const std::string& s, const std::string& e)
{
    if (s.size() < e.size())
        return false;
...
bool
endswith(const std::string& s, const std::string& e)
{
    if (s.size() < e.size())
        return false;
...
to_lower(const std::string& s)
{
    std::string res = s;
    std::transform(
        res.begin(), res.end(), res.begin(), [](unsigned char c) { return std::tolower(c); });
    return res;
}

char*
s2c(Allocator& al, const std::string& s)
{
    Str x;
    x.from_str_view(s);
    return x.c_str(al);
...
string_split(const std::string& s, const std::string& split_string, bool strs_to_lower)
{
...
std::vector<std::string>
string_split_avoid_parentheses(const std::string& str, bool strs_to_lower)
{
    std::vector<std::string> result;
    std::string word;
...
std::string
join(const std::string j, const std::vector<std::string>& l)
{
    std::string result;
...
std::vector<std::string>
slice(const std::vector<std::string>& v, int start, int end)
{
    int oldlen = v.size();
    int newlen;
...
std::string
replace(const std::string& s, const std::string& regex, const std::string& replace)
{
...
read_file(const std::string& filename, std::string& text)
{
    ...
    text = std::string(bytes.data(), filesize);
    return true;
}

std::string
read_file_ok(const std::string& filename)
{
    std::string text;
...
bool
is_relative_path(const std::string& path)
{
    return !startswith(path, "/");
}

std::string
join_paths(const std::vector<std::string>& paths)
{
    std::string p;
    std::string delim = "/";
    for (auto& path : paths) {
...
std::string
str_escape_c(const std::string& s)
{
    std::ostringstream o;
...
char*
str_unescape_c(Allocator& al, LCompilers::Str& s)
{
    std::string x = "";
    size_t idx = 0;
...
std::string
str_escape_fortran_double_quote(const std::string& s)
{
    std::ostringstream o;
    for (auto c = s.cbegin(); c != s.cend(); c++) {
        switch (*c) {
...
char*
str_unescape_fortran(Allocator& al, LCompilers::Str& s, char ch)
{
    std::string x = "";
    size_t idx = 0;
    for (; idx + 1 < s.size(); idx++) {
...
bool
str_compare(const unsigned char* pos, std::string s)
{
